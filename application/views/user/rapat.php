								...
								<table class=" table table-bordered table-hover" id="table-id" style="font-size:14px;">
									...
									<tbody>
										...
										<?php foreach ($list_rapat as $lu) : ?>
											<tr>
												<td><button type="button" class="tombol-edit btn btn-dark btn-sm" data-id="<?php echo $lu['id_rapat']; ?>" data-toggle="modal" data-target="#ubah-status"><i class="fas fa-location-arrow"></i></button></td>
												...
												<?php if ($lu['status_rapat'] == 1) : ?>
													<td>Belum Mulai</td>
												<?php elseif ($lu['status_rapat'] == 2) : ?>
													<td><button class="btn btn-warning btn-block btn-sm font-weight-bolder">Proses</button></td>
												<?php else : ?>
													<td><button class="btn btn-success btn-block btn-sm">Selesai</button></td>
												<?php endif; ?>
												<?php if ($lu['status_rapat'] == 1) : ?>
												...
												<?php else : ?>
													<td class="font-weight-bolder text-danger text-center">Closed</td>
												<?php endif; ?>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="box-body">
					<form action="<?php echo base_url('user/rapat'); ?>" method="post">
...
<div class="modal fade" id="edit-user">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="box-body">
					<form action="<?php echo base_url('user/edit_rapat'); ?>" method="post">
						<div class="form-group">
						...
						<div class="form-group">
							<label>Kode Rapat</label>
							<input type="text" class="form-control form-control-sm" id="kode_rapat" readonly>
						</div>
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
...
<div class="modal fade" id="ubah-status">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="box-body">
					<form action="<?php echo base_url('user/ubah_status'); ?>" method="post">

						<div class="form-group">
							<label>Kode Rapat</label>
							<input type="hidden" name="id_rapat" id="id_rapat_status">
							<input type="text" class="form-control form-control-sm" id="kode_rapat_status" readonly>
						</div>

						<div class="form-group">
							<label>Nama Rapat</label>
							<input type="text" class="form-control form-control-sm" id="nama_rapat_status" readonly>
						</div>
						<div class="form-group">
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio" name="status_rapat" value="2" required>
								<label class="form-check-label font-weight-bolder">Proses</label>
							</div>
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio" name="status_rapat" value="0">
								<label class="form-check-label font-weight-bolder">Selesai</label>
							</div>
						</div>
						<button type="submit" class="btn btn-primary mr-2">Simpan Data</button>
						<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
	...
	$('.tombol-edit').on('click', function() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				$('#id_rapat_status').val(data.id_rapat);
				$('#nama_rapat_status').val(data.nama_rapat);
				$('#kode_rapat_status').val(data.kode_rapat);
			}
		});
	});
</script>
